...
origen = casa.search_vertice('habitacion 1','nombre')
destino = casa.search_vertice('sala de estar', 'nombre')
camino = casa.dijkstra(origen,destino)

if origen and destino is not None:
    if casa.has_path(origen, destino, criterio='nombre') is not None:
        caminoCorto = casa.dijkstra(origen,destino)
        print('camino mas corto')
        while caminoCorto.size() > 0:
            value = caminoCorto.pop()
            if destino==value[0]:
